...
pub fn view_create(data: *u8, len: usize) String_View {
    return .{ .data = data, .len = len };
}

pub fn view_cmp(a: String_View, b: String_View) c_int {
    return if (a.len != b.len) @as(c_int, 0) else @intFromBool(!(strncmp(a.data, b.data, a.len) != 0));
}

pub fn view_to_cstr(view: String_View) [*c]u8 {
    const str: *u8 = @as(*u8, @ptrCast(@alignCast(malloc((@sizeOf(u8) *% view.len) +% @as(c_ulong, @bitCast(@as(c_long, @as(c_int, 1))))))));
    _ = strncpy(str, view.data, view.len);
    str[view.len] = '\x00';
    return str;
}

pub fn view_trim_left(view: String_View) String_View {
    var i: usize = 0;
...
pub fn view_trim_right(view: String_View) String_View {
    var i: usize = view.len - 1;
    while ((i < view.len) and ((@as(c_int, @bitCast(@as(c_uint, (blk: {
        const tmp = @as(c_int, @bitCast(@as(c_uint, view.data[i])));
        if (tmp >= 0) break :blk __ctype_b_loc().* + @as(usize, @intCast(tmp)) else break :blk __ctype_b_loc().* - ~@as(usize, @bitCast(@as(isize, @intCast(tmp)) +% -1));
    }).*))) & @as(c_int, @bitCast(@as(c_uint, @as(c_ushort, @bitCast(@as(c_short, @truncate(_ISspace)))))))) != 0)) {
...
pub fn view_contains(haystack: String_View, needle: String_View) c_int {
...
pub fn view_first_of(view: String_View, target: u8) usize {
    var i: usize = 0;
    while (i < view.len) : (i += 1) {
        if (view.data[i] == target) return i;
    }
    return 0;
}

pub fn view_last_of(view: String_View, target: u8) usize {
    var i: usize = view.len - 1;
    while (i > @as(usize, @bitCast(@as(c_long, @as(c_int, 0))))) : (i -= 1) {
        if (@as(c_int, @bitCast(@as(c_uint, view.data[i]))) == @as(c_int, @bitCast(@as(c_uint, target)))) return i;
    }
    return 0;
}

pub fn view_split(arg_view: String_View, arg_c: u8, arg_arr: *String_View, arg_arr_s: usize) usize {
    var view = arg_view;
    _ = &view;
    var c = arg_c;
    _ = &c;
...
pub fn view_chop(view: String_View, arg_c: u8) String_View {
    var c = arg_c;
    _ = &c;
    var i: usize = 0;
    _ = &i;
...
pub fn view_rev(view: String_View, arg_data: *u8, arg_data_s: usize) String_View {
    var data = arg_data;
    _ = &data;
...
pub fn view_find(arg_haystack: String_View, arg_needle: String_View) usize {
...
pub fn view_to_int(arg_view: String_View) c_int {
    var view = arg_view;
    _ = &view;
    return @as(c_int, @bitCast(@as(c_int, @truncate(strtol(view.data, null, @as(c_int, 10))))));
}

pub fn view_to_float(arg_view: String_View) f32 {
    var view = arg_view;
    _ = &view;
